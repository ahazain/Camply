...
$result = $conn->query("SELECT * FROM produk WHERE id_kategori=1 LIMIT $startIndex, $productPerPage");
$totalProducts = mysqli_num_rows($conn->query("SELECT * FROM produk WHERE id_kategori=1"));
?>
...
<nav aria-label="...">
    <ul class="pagination">
        ...
        if ($currentPage > 1) {
            ...
            echo "<li class='page-item'><a class='page-link' href='?url=produk/man&page=$prevPage'>Previous</a></li>";
        }
        ...
        for ($i = 1; $i <= $totalPages; $i++) {
            ...
            echo "<li class='page-item $activeClass'><a class='page-link' href='?url=produk/man&page=$i'>$i</a></li>";
        }

        // Menampilkan tombol Halaman Berikutnya jika tidak berada di halaman terakhir
        if ($currentPage < $totalPages) {
            $nextPage = $currentPage + 1;
            echo "<li class='page-item'><a class='page-link' href='?url=produk/man&page=$nextPage'>Next</a></li>";
        }
        ?>
    </ul>
</nav>